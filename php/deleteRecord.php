...
the code in catch-block is executed. */
    ...
    try {
        $sql = "DELETE FROM $dbname.Vehicle WHERE LicensePlate = :plate";
        $stmnt = $conn->prepare($sql);     // read about prepared statements here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':plate', $_POST['plate']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
