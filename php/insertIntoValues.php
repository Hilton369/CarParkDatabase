...
try {
	$sql = "INSERT INTO Vehicle (LicensePlate, VehicleType) VALUES (:plate, :vtype);";   // all the variable names must start with a colon (:)
	$stmnt = $conn->prepare($sql);    // read about prepared statement here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
	$stmnt->bindParam(':plate', $_POST['plate']);   // stdId in $_POST['stdId'] in the exact name of the input element in HTML. if any typo, your code does not work   
	$stmnt->bindParam(':vtype', $_POST['vtype']);   // note the single quotes, If you forget to put single quotes, your code does not work.
